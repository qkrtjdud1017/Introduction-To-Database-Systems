...
**Assigned date:** Wednesday, November 7th, 2018.

**Due date:** Tuesday, November 20th, 2018. You have 2 weeks for this homework.
...
Follow these steps to set up your Amazon Web Services account. **You should have already done this, but we just repeat the instructions here for your reference.**
...
Run `sudo ./installSpark.sh` in your `starter-code` directory in your computer. This will download Spark 2.2.2 and unzip it into `/usr/local` on your computer. You can also download it directly from [here](http://spark.apache.org/downloads.html) be careful to download the right version of Spark. You may also choose to unzip Spark onto another directory, although you will have to adjust the commands we provide accordingly.
...
(On Unix or OSX you need to put " " in the classpath, i.e., `"/usr/local/spark-2.2.2-bin-hadoop2.7/jars/*"`)

```sh
$ javac -cp /usr/local/spark-2.2.2-bin-hadoop2.7/jars/* HW6.java
```

You can now run the code as (use ";" instead of ":" as the classpath separator on Windows):

```sh
$ java -cp /usr/local/spark-2.2.2-bin-hadoop2.7/jars/*:. HW6 <path to the flights data directory> <path to output directory where you want the output to be stored>
```
