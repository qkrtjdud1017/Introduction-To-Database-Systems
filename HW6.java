...
public class HW6 {
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
	...
	d = d.filter(t -> t.get(ORIGIN_CITY_NAME).equals("Seattle, WA"))
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
	JavaPairRDD<Tuple2<String, Integer>, Integer> f = d.filter(t -> t.get(CANCELLED).equals(0))
		.mapToPair(t -> new Tuple2<Tuple2<String, Integer>, Integer>(new Tuple2<String, Integer> 
			(t.get(ORIGIN_CITY_NAME).toString(), (Integer)(t.get(MONTH))), 1));
    f = f.reduceByKey((v,w) -> v + w);
	return f;
  }

  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
	JavaRDD<Tuple2<String, Integer>> f = d.mapToPair(t -> new Tuple2<Tuple2<String, String>, Integer> 
		(new Tuple2<String, String>(t.get(ORIGIN_CITY_NAME).toString(), t.get(DEST_CITY_NAME).toString()), 1))
		.reduceByKey((v,w) -> v + w)
		.map(t -> new Tuple2<String, Integer>(t._1._1, t._2));
	Tuple2<String, Integer> m = f.max(new tupleComparator());
	return m;
  }
  
  private static class tupleComparator implements Serializable, Comparator<Tuple2<String, Integer>> {
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
	JavaRDD<Row> nonNull = d.filter(t -> t.get(DEP_DELAY) != null);
	JavaPairRDD<String, Double> f = nonNull
		.mapToPair(t -> new Tuple2<String, Double> (t.get(ORIGIN_CITY_NAME).toString(), Double.valueOf(t.get(DEP_DELAY).toString())))
	    .reduceByKey((v, w) -> v + w);
	JavaPairRDD<String, Double> count = nonNull
		.mapToPair(t -> new Tuple2<String, Double> (t.get(ORIGIN_CITY_NAME).toString(), 1.0))
	    .reduceByKey((v, w) -> v + w);
	JavaPairRDD<String, Tuple2<Double, Double>> j = f.join(count);
	JavaPairRDD<String, Double> avg = j.mapToPair(t -> new Tuple2<String, Double> (t._1, t._2._1 / t._2._2));
    return avg;
  }
